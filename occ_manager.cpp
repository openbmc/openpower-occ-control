...
const auto MPIPL_REBOOT_FILE = "/run/openbmc/mpreboot@0";
...
#else
    if (!fs::exists(HOST_ON_FILE) && !fs::exists(MPIPL_REBOOT_FILE))
    {
    ...
    else
    {
        log<level::INFO>(
            fmt::format(
                ...
                fs::exists(HOST_ON_FILE) ? HOST_ON_FILE : MPIPL_REBOOT_FILE)
                .c_str());
        discoverTimer->restartOnce(10s);
    }
#endif
