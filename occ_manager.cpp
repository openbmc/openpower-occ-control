...
void Manager::readExtnSensors(const fs::path& path, uint32_t id)//SHELDON: Start
{
    ...
    for (auto& file : fs::directory_iterator(path))
    {
        ...
        // Grab 1 hex byte from labelValue string at a time and convert to ascii string.
        //    add that to the end of the sensor path.
        ...
        // Label indicating byte 5 and 6 is the current (mem,proc) power in Watts.
        if (("PWRM" == asciiLabelString) ||
            ("PWRP" == asciiLabelString))
        {
            ...
            // For Power field, Convert last 4 bytes of hex string into number value.
            ...
            MyHexNumber = std::round(((MyHexNumber / (PS_DERATING_FACTOR / 100.0))));
            log<level::ERR>(std::format("OCC{}:{} @ FILE:{} -- {} AC Watts", id, asciiLabelString,  filePathString + inputSuffix, MyHexNumber).c_str());

            dbus::OccDBusSensors::getOccDBus().setUnit(
                sensorPath, "xyz.openbmc_project.Sensor.Value.Unit.Watts");

            dbus::OccDBusSensors::getOccDBus().setValue(sensorPath,MyHexNumber);
...
void Manager::getSensorValues(std::unique_ptr<Status>& occ)
{
    ...
    if (fs::exists(sensorPath))
    {
        ...
        readExtnSensors(sensorPath, id);//SHELDON:
