...
void OccPersistCapData::save()
{
    ...
    try
    {
        ...
        lg2::info("OccPersistCapData::save() wrote {LEN} bytes from {PATH}",
                  "LEN", stream.tellp(), "PATH", opath);
    }
...
void OccPersistCapData::load()
{
    ...
    try
    {
        ...
        lg2::info("OccPersistCapData::load() read {LEN} bytes from {PATH}",
                  "LEN", stream.gcount(), "PATH", ipath);
