...
/** @brief Default time, in seconds, between OCC temp poll commands */
constexpr uint8_t defaultTempPollingInterval = 1;
...
void Manager::statusCallBack(bool status)
{
    ...
    if ((!_tempPollTimer->isEnabled()) && (activeCount > 0))
    {
        log<level::INFO>(
            fmt::format("Manager::statusCallBack(): {} OCCs temp will "
                        "be polled every {} seconds",
                        activeCount, defaultTempPollingInterval)
                .c_str());

        // Send temp poll and start OCC temp poll timer
        tempPollerTimerExpired();
    }
    else if ((_tempPollTimer->isEnabled()) && (activeCount == 0))
    {
        // Stop OCC temp poll timer
        log<level::INFO>("Manager::statusCallBack(): OCCs are not running, "
                         "stopping temp poll timer");

        _tempPollTimer->setEnabled(false);
    }
}
...
void Manager::tempPollerTimerExpired()
{
    if (activeCount == 0)
    {
        // No OCCs running, so poll timer will not be restarted
        log<level::INFO>(
            "Manager::tempPollerTimerExpired(): No OCCs running, temp poll "
            "timer not restarted");
    }
    ...
    {
        log<level::ERR>(
            "Manager::tempPollerTimerExpired() ERROR: Temp timer not defined");
        return;
    }

    getProcDimmTemp();

    // Restart OCC temp poll timer
    _tempPollTimer->restartOnce(
        std::chrono::seconds(defaultTempPollingInterval));
}
