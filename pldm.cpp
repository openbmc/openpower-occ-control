                         ...
                         const uint8_t instance, const bool rspExpected)
{
    ...
    if (rspExpected)
    {
        ...
        log<level::INFO>(
            fmt::format(
                "checkActiveSensor: calling pldm_send(OCC{}, instance:{}, {} bytes)",
        ...
        if (pldmRc != PLDM_REQUESTER_SUCCESS)
        {
            log<level::ERR>(
                fmt::format(
                    "checkActiveSensor: pldm_send failed with rc={} and errno={}/{}",
...
void Interface::registerPldmRspCallback()
{
    ...
    log<level::INFO>(
        fmt::format("registerPldmRspCallback(fd={})", pldmFd).c_str());
    ...
    else
    {
        ...
        log<level::INFO>(
            fmt::format("registerPldmRspCallback: sd_event_add_io success")
                .c_str());
...
void Interface::pldmRspExpired()
{
    if (!pldmResponseReceived)
    {
        ...
        // TODO: DISABLE OCC???
        log<level::WARNING>("pldmRspExpired: disabling OCC active sensor");
        callBack(pldmResponseOcc, false);
...
{
    log<level::INFO>(fmt::format("pldmClose: Closing fd({})", pldmFd).c_str());
    close(pldmFd);
    pldmFd = -1;
    eventSource.reset();
}
