...
void Status::resetOCCDbus()
{
    ...
    try
    {
        ...
        method.append(tid);
        method.append(std::variant<uint16_t>(PLDM_ENTITY_PROC_MODULE));
        method.append(
            std::variant<uint16_t>(PLDM_STATE_SET_BOOT_RESTART_CAUSE));
        auto reply = bus.call(method);
        reply.read(pdrList);
    }
    catch (const std::exception& e)
    {
        log<level::ERR>("FindStateEffecterPDR returned error, ",
    ...
    resetOCCPLDM(pdrList, mctpEid, instanceId, entityInstance);
...
#ifdef ENABLE_PLDM
std::vector<uint8_t> prepareSetStateEffecterReq(pldm_state_effecter_pdr* pdr,
                                                uint8_t instanceId)
{
    assert(pdr->composite_effecter_count != 0);
    auto pStates = pdr->possible_states;
    auto possibleStates =
        reinterpret_cast<state_effecter_possible_states*>(pStates);
    std::vector<set_effecter_state_field> stateField(
        pdr->composite_effecter_count * sizeof(set_effecter_state_field));

    std::vector<uint8_t> request(
        sizeof(pldm_msg_hdr) + sizeof(pdr->effecter_id) +
        sizeof(pdr->composite_effecter_count) + stateField.size());
    auto requestMsg = reinterpret_cast<pldm_msg*>(request.data());
    for (uint8_t effecters = 1; effecters <= pdr->composite_effecter_count;
         effecters++)
    {
        if (possibleStates->state_set_id == PLDM_STATE_SET_BOOT_RESTART_CAUSE)
        {
            stateField[effecters - 1].set_request = PLDM_REQUEST_SET;
            stateField[effecters - 1].effecter_state =
                PLDM_STATE_SET_PREDICTIVE_CONDITION;
        }
        else
        {
            stateField[effecters - 1].set_request = PLDM_NO_CHANGE;
            stateField[effecters - 1].effecter_state = 0;
        }
        pStates += possibleStates->possible_states_size + sizeof(uint16_t) +
                   sizeof(possibleStates->possible_states_size);
    }
    auto rc = encode_set_state_effecter_states_req(
        instanceId, pdr->effecter_id, pdr->composite_effecter_count,
        stateField.data(), requestMsg);
    if (rc != PLDM_SUCCESS)
    {
                  ...
                  uint8_t instanceId, uint16_t entityInstance)
{
    ...
    {
        auto pdr = reinterpret_cast<pldm_state_effecter_pdr*>(pdrVec.data());

        if (pdr->hdr.type == PLDM_STATE_EFFECTER_PDR &&
            pdr->entity_instance == entityInstance)
        {
            auto stateEffecReqMsg = prepareSetStateEffecterReq(pdr, instanceId);
            size_t pdrResponseMsgSize{};
            auto pdrResponseMsg =
