...
bool Interface::freePldmInstanceId()
{
    ...
    if (pldmInstanceID)
        log<level::ERR>(std::format("freed").c_str());
    else
        log<level::ERR>(std::format("failed to freed").c_str());

    return true;
}

int Interface::setupPldmTransport()
{
    ...
    if (pldmTransport)
    {
        log<level::ERR>(
            std::format(
                "setupPldmTransport: init pldm_transport already setup!, errno={}/{}",
    ...
    if (rc)
    {
        log<level::ERR>(
            std::format(
                "setupPldmTransport: Failed to init MCTP demux transport, errno={}/{}",
                rc, strerror(rc))
                .c_str());
        return -1;
    }
    ...
    if (pldm_transport_mctp_demux_map_tid(mctpDemux, tid, mctpEid))
    {
        log<level::ERR>(
            std::format(
                "setupPldmTransport: Failed to setup tid to eid mapping, errno={}/{}",
                openErrno, strerror(openErrno))
                .c_str());
        pldmClose();
        return -1;
    ...
    {
        log<level::ERR>(
            std::format("setupPldmTransport: Failed to get pollfd , errno={}/{}",
                        openErrno, strerror(openErrno))
                .c_str());
        pldmClose();
        return -1;
    ...
    pldmFd = pollfd.fd;
    log<level::ERR>(std::format("setupPldmTransport: pldmFd has fd={}", pldmFd).c_str());
    return 0;
}
                         ...
                         const uint8_t instance, const bool rspExpected)
{
    ...
    auto rc = setupPldmTransport();
    if (rc)
    {
        log<level::ERR>(
            std::format("sendPldm: setupPldmTransport failed{}", rc).c_str());
    }
