...
void Status::resetOCC()
{
    ...
    try
    {
        ...
        {
            std::cout << "PDR header is of type state effecter" << std::endl;
            ...
            for (uint8_t effecters = 0x01; effecters <= compositeEffecterCount;
                 effecters++)
            {
                std::cout << effecters << " effecter count" << std::endl;
                if (possibleStates->state_set_id == stateSetId &&
                    possibleStates->possible_states_size != 0x00)
                {
                    std::cout << "found effecter with state ID " << stateSetId
                              << std::endl;
                    ...
                    try
                    {
                        ...
                        std::cout << "Got instance ID " << stateSetId
                                  << std::endl;
