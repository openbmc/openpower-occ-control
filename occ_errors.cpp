...
// Since its a /sysfs file, data would be in string
                         ...
                         uint32_t revents, void* userData)
{
    ...
    // If data is `0\n`, then it's success. Failure otherwise
    if (error->readFile() != NO_ERROR && error->callBack)
    {
        error->callBack();
    }
    return 0;
...
// then its an error
std::string Error::readFile()
{
    char data[256]{};

    // By far, the max we may get is under 8 characters
