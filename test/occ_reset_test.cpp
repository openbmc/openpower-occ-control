...
TEST(parseStateEffecterData, testOccReset)
{
    ...
    const uint8_t compositeEffecterCount = 2;
    ...
    possibleStates->states[1].byte = bf1.byte;

    auto requestMsg = prepareSetStateEffecterReq(record, instanceId);
    auto request = reinterpret_cast<pldm_msg*>(requestMsg.data());

    uint16_t retEffecterId = 0;
    uint8_t retCompEffecterCnt = 0;

    std::array<set_effecter_state_field,
    ...
    EXPECT_EQ(retStateField[0].effecter_state, possibleStates->states[0].byte);
    EXPECT_EQ(retStateField[1].set_request, PLDM_REQUEST_SET);
    EXPECT_EQ(retStateField[1].effecter_state, possibleStates->states[1].byte);
}
