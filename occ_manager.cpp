...
void Manager::readExtnSensors(const fs::path& path, uint32_t id)
{
    ...
    for (auto& file : fs::directory_iterator(path))
    {
        ...
        if ((labelValue == EXTN_LABEL_PWRM_MEMORY_POWER) ||
            (labelValue == EXTN_LABEL_PWRP_MEMORY_POWER))
        {
            // Build the dbus String for this chiplet power asset.
            if (labelValue == EXTN_LABEL_PWRP_MEMORY_POWER)
            {
                labelValue = "_power";
            }
            ...
            catch (const std::system_error& e)
            {
                lg2::debug(
                    ...
                    "PATH", filePathString + inputSuffix, "ERROR", e.code().value());
                continue;
            }
            ...
            lg2::debug("OCC{ID}: FILE:{FILE} -- {ACWATTS} AC Watts",
                "ID", id,
                "FILE", filePathString + inputSuffix,
                "ACWATTS", MyHexNumber);
