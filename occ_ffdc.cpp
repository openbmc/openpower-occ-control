                         ...
                         int fd)
{
    ...
    log<level::INFO>(
        fmt::format("Creating PEL for OCC{} with SBE FFDC: {} - SRC6: 0x{:08X}", src6 >> 16,
                    path, src6)
            .c_str());
                             ...
                             const char* callout)
{
    ...
    log<level::INFO>(
        fmt::format("Creating OCC Reset PEL for OCC{}: {}", instance,
                    path)
            .c_str());
    ...
    {
        log<level::ERR>(
            fmt::format("Failed to create OCC Reset PEL: {}", e.what()).c_str());
    }
}
