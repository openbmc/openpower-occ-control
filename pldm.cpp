                         ...
                         const uint8_t instance, const bool rspExpected)
{
    ...
    if (rspExpected)
    {
        ...
        auto pldmRc =
            pldm_transport_send_msg(pldmTransport, pldmTID, request.data(), request.size());
    ...
    else // not expecting the response
    {
        ...
        auto rc = pldm_transport_send_msg(pldmTransport, pldmTID, request.data(), request.size());
        auto sendErrno = errno;
        if (rc)
        {
            log<level::ERR>(
                               ...
                               uint32_t revents, void* userData)
{
    ...
    {
        log<level::INFO>(
            std::format("pldmRspCallback: calling pldm_transport_recv_msg() instance:{}",
    ...
    {
        log<level::INFO>(
            std::format("pldmRspCallback: pldm_transport_recv_msg() rsp was {} bytes",
                        responseMsgSize)
                .c_str());
    }
