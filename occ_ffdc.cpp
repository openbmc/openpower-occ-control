...
void FFDC::analyzeEvent()
{
    ...
    {
        uint32_t* wordData = reinterpret_cast<uint32_t*>(data.get());
        log<level::INFO>("OCC SBE error detected with no FFDC",
                         entry("WORD1=%08x", wordData[0]),
                         entry("WORD2=%08x", wordData[1]));
        return;
    }
