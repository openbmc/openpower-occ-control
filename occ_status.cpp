...
CmdStatus Status::sendModeChange()
{
    ...
    if (device.master())
    {
        ...
        {
            log<level::INFO>(fmt::format("Status::sendModeChange: Sending "
                                         "CHANGE_MODE to master OCC{}",
                                         instance)
                                 .c_str());
            ...
            if (status == CmdStatus::SUCCESS)
            {
                if (rsp.size() == 5)
                {
                    if (CmdType::SET_MODE_AND_STATE == CmdType(rsp[1]))
                    {
                    ...
                    else
                    {
                        log<level::ERR>(
                            fmt::format(
                                "Status::sendModeChange: SET MODE response "
                                "command mismatch"
...
CmdStatus Status::sendIpsData()
{
    ...
    if (device.master())
    {
        ...
        if (status == CmdStatus::SUCCESS)
        {
            if (rsp.size() == 5)
            {
                if (CmdType::SET_CONFIG_DATA == CmdType(rsp[1]))
                {
                    if (RspStatus::SUCCESS == RspStatus(rsp[2]))
                    {
                        log<level::INFO>(
