...
std::optional<std::string>
    Manager::analysisPowerLabelValue(const std::string& value1)
{
    if (value == "system")
    {
        return value;
...
void Manager::readPowerSensors(const fs::path& path, uint32_t id)
{
    ...
    for (auto& file : fs::directory_iterator(path))
    {
        ...
        auto labelValue = analysisPowerLabelValue(state);
        if (labelValue == std::nullopt)
        {
            continue;
        }

        // const std::string& powerLabelValue = *labelValue;
        ...
        std::map<std::string, std::string>::iterator iter =
            powerSensorName.find(*labelValue);
...
void Manager::getSensorValues(uint32_t id)
{
