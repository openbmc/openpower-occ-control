                                  ...
                                  std::vector<std::int32_t>& response) const
{
    ...
    if ((target_instance < activeCount) && (command.size() >= 3) &&
        (statusObjects[target_instance]->occ_pt != nullptr))
    {
        status =
            statusObjects[target_instance]->occ_pt->send(command, response);
        if (status == SUCCESS)
        {
            if (response.size() >= 7)
            {
    ...
    {
        log<level::ERR>(fmt::format("Manager::sendOccCommand: Send of command "
                                    "failed with status {}",
                                    status)
                            .c_str());
    }
