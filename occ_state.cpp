...
void StateChange::analyzeEvent()
{
    ...
    if (r < 0)
    {
        log<level::ERR>(fmt::format("StateChange::analyzeEvent: OCC State "
                                    "Change Detected (failing ioctl: rc={})",
                                    r)
                            .c_str());
        elog<ConfigFailure>(
    ...
    if (data.empty())
    {
        log<level::ERR>("StateChange::analyzeEvent: OCC State Change Detected "
                        "(empty data)");
        return;
    }
    ...
    log<level::INFO>(
        fmt::format(
            "StateChange::analyzeEvent: Kernel detected OCC State Change ({})",
            newState)
            .c_str());
