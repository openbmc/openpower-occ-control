...
template <typename T>
T readFile(const std::string& path) // SHELDON:  could this be standard utility?
{
    std::ifstream ifs;
...
void Manager::findAndCreateObjects() // SHELDON: Inherited
{
    if (!pmode)
    {
...
void Manager::initTimerObjects()
{
    ...
    discoverTimer = std::make_unique<
        sdeventplus::utility::Timer<sdeventplus::ClockId::Monotonic>>(
        sdpEvent, std::bind(&Manager::findAndCreateObjects, this));

    waitForAllOccsTimer = std::make_unique<
        sdeventplus::utility::Timer<sdeventplus::ClockId::Monotonic>>(
        sdpEvent, std::bind(&Manager::occsNotAllRunning, this));

    throttlePldmTraceTimer = std::make_unique<
        sdeventplus::utility::Timer<sdeventplus::ClockId::Monotonic>>(
...
void Manager::sbeHRESETResult(instanceID instance, bool success) // SHELDON:
// SHELDON: ?? Quesiton the PHAL_SUPPORT on which driver.
{
    if (success)
    {
...
#ifdef PHAL_SUPPORT
bool Manager::sbeCanDump(unsigned int instance) // SHELDON: ?? PHAL_SUPPORT ONLY
{
...
void Manager::setSBEState(unsigned int instance, enum sbe_state state) // SHELDON:
// SHELDON: ???? PHAL_SUPPORT ONLY
{
    struct pdbg_target* proc = getPdbgTarget(instance);
...
struct pdbg_target* Manager::getPdbgTarget(unsigned int instance) // SHELDON:
// SHELDON: ???? PHAL_SUPPORT ONLY
{
    if (!pdbgInitialized)
    {
