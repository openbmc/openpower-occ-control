...
struct Manager
{
  ...
  private:
     ...
     */
    void sbeHRESETResult(instanceID instance, bool success); // SHELDON: NEW P10-11 ++
                                    // ?? Quesiton the PHAL_SUPPORT on which driver.
#ifdef PHAL_SUPPORT
     ...
     */
    bool sbeCanDump(unsigned int instance); // SHELDON: NEW P10-11 ++
                                    //  ???? PHAL_SUPPORT ONLY
     ...
     *
     */
    void setSBEState(unsigned int instance, enum sbe_state state); // SHELDON: NEW P10-11 ++
                                    // ???? PHAL_SUPPORT ONLY
     ...
     */
    struct pdbg_target* getPdbgTarget(unsigned int instance); // SHELDON: NEW P10-11 ++
                                    // ???? PHAL_SUPPORT ONLY
