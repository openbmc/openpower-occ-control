...
int Interface::setupPldm()
{
    ...
    int rc = pldm_transport_mctp_demux_init(&mctp_demux);
    if (rc)
    {
        log<level::ERR>(
            std::format(
    ...
    if (pldm_transport_mctp_demux_map_tid(mctp_demux, tid, mctpEid))
    {
        log<level::ERR>(
            std::format(
    ...
    {
        log<level::ERR>(
            std::format("setupPldm: Failed to get pollfd , errno={}/{}",
                        openErrno, strerror(openErrno))
                .c_str());
        pldmClose();
        return -1;
    ...
    pldmFd = pollfd.fd;
    log<level::ERR>(std::format("setupPldm: pldmFd has fd={}", pldmFd).c_str());
    return 0;
}
                         ...
                         const uint8_t instance, const bool rspExpected)
{
    ...
    {
        log<level::ERR>(
            std::format("sendPldm: setupPldm failed{}", rc).c_str());
    }
