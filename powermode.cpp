...
// Set the master OCC path (so cmds can get sent to the OCC)
bool PowerMode::setMaster(const char* masterOccPath)
{
    log<level::INFO>(fmt::format("PowerMode::setMaster(OCC{}) created={}",
                                 masterOccPath, occCmd ? "y" : "n")
                         .c_str());
    if (!occCmd)
    {
        path = masterOccPath;
        occInstance = this->path.back() - '0';
        log<level::INFO>(fmt::format("PowerMode::setMaster(OCC{}, {})",
    ...
    {
        log<level::ERR>(
            fmt::format("PowerMode::setMaster: Master already set (OCC{}, {})",
                        occInstance, masterOccPath)
                .c_str());
        return false;
    }
...
void OccPersistData::print()
{
    ...
    log<level::INFO>(
        fmt::format(
            ...
            modeData.ipsEnabled, modeData.ipsEnterUtil, modeData.ipsEnterTime,
            modeData.ipsExitUtil, modeData.ipsExitTime)
            .c_str());
}
